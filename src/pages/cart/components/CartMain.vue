...
} from '@/services/member/cart'
/** 类型 */
import type { CartResult, CartItem } from '@/types/member/cart'
...
/** 获取购物车列表数据 */
const cartList = ref<CartResult>([]) // 保存购物车列表数据
...
/************ 修改选中状态 ************/
// item选中点击事件
const onChangeSelectedClick = (item: CartItem) => {
  // 修改当前item选中状态
...
// 全选按钮点击事件
const onChangeSelectedAllClick = () => {
  // 全选状态取反
...
<template>
  <scroll-view scroll-y class="scroll-view" @scrolltolower="onScrollToLower">
    ...
    <template v-if="memberStore.profile">
      ...
      <view class="cart-list" v-if="cartList.length">
        ...
        <uni-swipe-action>
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <view class="goods">
              <!-- 选中状态 -->
              <text @tap="onChangeSelectedClick(item)" class="checkbox" :class="{ checked: item.selected }"></text>
              <navigator :url="`/pages/goods/goods?id=${item.id}`" hover-class="none" class="navigator">
                <image mode="aspectFill" :src="item.picture" class="picture" />
                <view class="meta">
              ...
              <!-- 商品数量 -->
              <view class="count">
                <vk-data-input-number-box :index="item.skuId" v-model="item.count" :min="1" :max="item.stock"
            ...
            <template #right>
              <view class="cart-swipe-right">
                <button @tap="deleteCartItem(item.skuId)" class="button delete-button">删除</button>
              </view>
            </template>
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar" :style="props.isPage && safeAreaBottom">
        <text @tap="onChangeSelectedAllClick" class="all" :class="{ checked: isSelectedAll }">全选</text>
